...
//       customDisplayName ?? handle.displayName.split(" ").first;
...
//         LangToolService(
//             LanguageToolClient(language: ss.settings.spellcheckLanguage.value)),
...
//   Mistake? get selectedMistake => _selectedMistakeIndex == -1
//       ? null
//       : _mistakes.elementAtOrNull(_selectedMistakeIndex);
...
//       String emojiName =
//           newText.substring(match.start + 1, match.end - 1).toLowerCase();
...
//       return;
//     }
//     final mistakeIndex = _mistakes.indexWhere((e) =>
//         (e.offset == newSelection.baseOffset) &&
//         (e.endOffset == newSelection.extentOffset));
//     if (mistakeIndex != -1) {
...
//   OverlayEntry _createTooltip(BuildContext context, Offset offset,
//       Mistake mistake, String mistakeText) {
...
//           .where((e) => e.offset >= offset && e.endOffset <= endOffset)
//           .toList();
...
//             backgroundColor: _getMistakeColor(mistake.type)
//                 .withOpacity(highlightStyle.backgroundOpacity),
...
//           final prevMistakeEnd =
//               i == 0 ? 0 : mistakes[i - 1].endOffset - offset;
//           final leadingNonMistakeText =
//               chunk.substring(prevMistakeEnd, mistakeStart);
//           if (leadingNonMistakeText.isNotEmpty)
//             spans.add(TextSpan(text: leadingNonMistakeText, style: style));
...
//                     context,
//                     Offset(event.position.dx - ns.widthChatListLeft(context),
//                         event.position.dy),
//                     mistake,
//                     mistakeText);
...
//             final nextMistakeStart = i == mistakes.length - 1
//                 ? chunk.length
//                 : mistakes[i + 1].offset - offset;
//             final trailingNonMistakeText =
//                 chunk.substring(mistakeEnd, nextMistakeStart);
//             if (trailingNonMistakeText.isNotEmpty)
//               spans.add(TextSpan(text: trailingNonMistakeText, style: style));
//           }
//         }
...
//         context: context, chunk: text, offset: 0, style: style);
//   }
// }
...
//     final replacement =
//         "$escapingChar$index$escapingChar${addSpace ? " " : ""}";
//     text = textParts[0] +
//         textParts[1].replaceFirst(candidate, replacement) +
//         textParts[2];
//     selection = TextSelection.collapsed(
//         offset: indexSelection - candidate.length + replacement.length);
//     processMentions();
//   }
...
//         if (MentionTextEditingController.escapingChar
//                     .allMatches(textPart1)
//                     .length %
//                 2 !=
//             0) {
//           final badMentionIndex =
//               textPart1.lastIndexOf(MentionTextEditingController.escapingChar);
//           textPart1 = textPart1.substring(0, badMentionIndex);
//           deletingBadMention = true;
//         }
//         if (MentionTextEditingController.escapingChar
//                     .allMatches(textPart2)
//                     .length %
//                 2 !=
//             0) {
//           final badMentionIndex =
//               textPart2.indexOf(MentionTextEditingController.escapingChar);
...
//                       baseOffset: offset - 1,
//                       extentOffset: offset + word.length + 1);
//                 }
//               },
//               child: ShaderMask(

...
ContactsService cs = Get.isRegistered<ContactsService>()
...
class ContactsService extends GetxService {
  ...
  Future<List<List<int>>> refreshContacts() async {
    ...
    if (!kIsWeb) {
      ...
      for (Contact c in dbContacts) {
        final refreshedContact =
            _contacts.firstWhereOrNull((element) => element.id == c.id);
      ...
      final newContacts = _contacts
          .where((e) => !dbContacts.map((e2) => e2.id).contains(e.id))
          .toList();
    ...
    for (Contact c in _contacts) {
      ...
      final addressesAndServices =
          handles.map((e) => e.uniqueAddressAndService).toList();
      if (handles.isNotEmpty) {
        handlesToSearch.removeWhere(
            (e) => addressesAndServices.contains(e.uniqueAddressAndService));

        // we have changes if the handle doesn't have an associated contact,
        // even if there were no contact changes in the first place
        final matches = handles.where(
            (e) => addressesAndServices.contains(e.uniqueAddressAndService));
  ...
  void completeContactsRefresh(List<Contact> refreshedContacts,
      {List<List<int>>? reloadUI}) {
        ...
        if (response.statusCode == 200 &&
          ...
          for (Map<String, dynamic> map in response.data['data']) {
            ...
            networkContacts.add(Contact(
              id: (map['id'] ?? (phones.isNotEmpty ? phones : emails))
                  .toString(),
    ...
    try {
      if (kIsWeb) {
        ...
        if (!isNullOrEmpty(response.data['data'])) {
          logger?.call("Found contacts!");
          for (Map<String, dynamic> map in response.data['data']
              .where((e) => !isNullOrEmpty(e['avatar']))) {
            final displayName = getDisplayName(
                map['displayName'], map['firstName'], map['lastName']);
            ...
            for (Contact contact in networkContacts) {
              bool match = contact.id ==
                  (map['id'] ?? (phones.isNotEmpty ? phones : emails))
                      .toString();

              // Ensure contact first name matches to avoid issues with shared numbers (landlines)
              if (!match &&
                  map['firstName'] != null &&
                  !contact.displayName.startsWith(map['firstName'])) {
                continue;
              }
              ...
              for (String a in addresses) {
                ...
                String? formatA = a.contains("@")
                    ? a.toLowerCase()
                    : await formatPhoneNumber(cleansePhoneNumber(a));
                if (formatA.isEmpty) continue;
                for (String _a in _addresses) {
                  String? _formatA = _a.contains("@")
                      ? _a.toLowerCase()
                      : await formatPhoneNumber(cleansePhoneNumber(_a));
                  if (formatA == _formatA) {
                    match = true;
                    break;
                  }
        ...
        if (response.statusCode == 200 &&
          ...
          for (Map<String, dynamic> map in response.data['data']) {
            ...
            networkContacts.add(Contact(
              ...
              emails: emails,
              phones: phones,
              avatar: !isNullOrEmpty(map['avatar'])
                  ? base64Decode(map['avatar'].toString())
                  : null,
            ));
          }
        } else {

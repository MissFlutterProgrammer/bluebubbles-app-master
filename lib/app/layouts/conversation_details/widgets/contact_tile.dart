  ...
  Widget build(BuildContext context) {
    ...
    final child = InkWell(
      ...
      child: ListTile(
        ...
        title: RichText(
          text: TextSpan(
            children: MessageHelper.buildEmojiText(
                handle.displayName, context.theme.textTheme.bodyLarge!),
          ),
        ),
        subtitle: contact == null || hideInfo
            ? null
            : Text(
        ...
        trailing: kIsWeb ||
                (kIsDesktop && !isEmail) ||
                (!isEmail && (contact?.phones.isEmpty ?? true))
            ? Container(width: 2)
            : FittedBox(
                child: Row(
                      ...
                      ButtonTheme(
                        ...
                        child: TextButton(
                          style: TextButton.styleFrom(
                            ...
                            backgroundColor: ss.settings.skin.value != Skins.iOS
                                ? null
                                : context.theme.colorScheme.secondary,
                          ),
                          onLongPress: () => showAddressPicker(
                              contact, handle, context,
                              isEmail: true, isLongPressed: true),
                          onPressed: () => showAddressPicker(
                              contact, handle, isEmail: true, context),
                          child: Icon(
                              ...
                              size: ss.settings.skin.value != Skins.iOS
                                  ? 25
                                  : 20),
                        ),
                      ),
              ...
              children: [
                SlidableAction(
                  ...
                  onPressed: (_) async {
                    ...
                    http
                        .chatParticipant("remove", chat.guid, handle.address)
                        .then((response) async {
                      Get.back();
                      ...
                      Logger.error(
                          "Failed to remove participant ${handle.address}",
                          error: err,
                          trace: stack);

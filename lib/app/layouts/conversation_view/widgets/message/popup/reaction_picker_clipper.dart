...
class ReactionPickerClipper extends CustomClipper<Path> {
  ...
  const ReactionPickerClipper(
      {required this.messageSize, required this.isFromMe});
  ...
  Path getClip(Size size) {
    ...
    path.arcToPoint(Offset(size.width - 20, size.height - 15),
        radius: const Radius.circular(20));
    path.lineTo(20, size.height - 15);
    path.arcToPoint(Offset(0, size.height - 35),
        radius: const Radius.circular(20));
    path.lineTo(0, 20);
    ...
    if (size.width > messageSize.width && ss.settings.skin.value == Skins.iOS) {
      if (isFromMe) {
        path.addArc(
            Rect.fromLTWH(
                size.width - messageSize.width, size.height - 22.5, 17.5, 17.5),
            0,
            2 * pi);
        path.addArc(
            Rect.fromLTWH(
                size.width - messageSize.width - 5, size.height - 7.5, 7, 7),
            0,
            2 * pi);
      } else {
        path.addArc(
            Rect.fromLTWH(
                messageSize.width - 20, size.height - 22.5, 17.5, 17.5),
            0,
            2 * pi);
        path.addArc(
            Rect.fromLTWH(messageSize.width - 5, size.height - 7.5, 7, 7),
            0,
            2 * pi);
      }
    }

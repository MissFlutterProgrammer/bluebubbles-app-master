...
class TailClipper extends CustomClipper<Path> {
  ...
  Path getClip(Size size) {
    ...
    if (!isFromMe && (showTail && ss.settings.skin.value == Skins.iOS)) {
      ...
      path.arcToPoint(Offset(start + 6.547, size.height - 5.201),
          radius: const Radius.circular(20), clockwise: false);
      path.arcToPoint(Offset(start + 20, size.height),
          radius: const Radius.circular(20), clockwise: false);
    } else {
      if (connectLower && !isFromMe) {
        path.lineTo(start, size.height - 5);
        path.arcToPoint(Offset(start + 5, size.height),
    ...
    if (isFromMe && (showTail && ss.settings.skin.value == Skins.iOS)) {
      ...
      // intersect slightly more than 45 deg on the arc
      path.arcToPoint(Offset(end - 6.547, size.height - 5.201),
          radius: const Radius.circular(20), clockwise: false);
      path.arcToPoint(Offset(size.width, size.height),
          radius: const Radius.circular(20), clockwise: false);
      path.arcToPoint(Offset(end, size.height - 10),
          radius: const Radius.circular(10));
    } else {
      if (connectLower && isFromMe) {
        path.lineTo(end - 5, size.height);
        path.arcToPoint(Offset(end, size.height - 5),
            radius: const Radius.circular(5), clockwise: false);
      } else {
        path.lineTo(end - 20, size.height);
        path.arcToPoint(Offset(end, size.height - 20),
            radius: const Radius.circular(20), clockwise: false);
      }
    }
    if (connectUpper && ss.settings.skin.value != Skins.iOS) {
      if (isFromMe) {
        path.lineTo(end, 5);
        path.arcToPoint(Offset(end - 5, 0),
            radius: const Radius.circular(5), clockwise: false);
        path.lineTo(start + 20, 0);
        path.arcToPoint(Offset(start, 20),
            radius: const Radius.circular(20), clockwise: false);
      } else {
        path.lineTo(end, 20);
        path.arcToPoint(Offset(end - 20, 0),
            radius: const Radius.circular(20), clockwise: false);
        path.lineTo(start + 5, 0);
        path.arcToPoint(Offset(start, 5),
            radius: const Radius.circular(5), clockwise: false);
      }
    } else {
      path.lineTo(end, 20);

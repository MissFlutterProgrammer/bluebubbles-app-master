...
class _ConversationPanelState extends OptimizedState<ConversationPanel> {
  ...
  void initState() {
    ...
    if (kIsDesktop) {
      ...
      (sendPlayer as Player)
          .stream
          .playing
          .listen((value) => playingSendSound.value = value);
      (receivePlayer as Player)
          .stream
          .playing
          .listen((value) => playingReceiveSound.value = value);
    } else {
      sendPlayer = aw.PlayerController();
      receivePlayer = aw.PlayerController();
      (sendPlayer as aw.PlayerController).onPlayerStateChanged.listen(
          (value) => playingSendSound.value = value == aw.PlayerState.playing);
      (receivePlayer as aw.PlayerController).onPlayerStateChanged.listen(
          (value) =>
              playingReceiveSound.value = value == aw.PlayerState.playing);
    }
  }
                ...
                children: [
                  ...
                  Obx(
                    () => SettingsSwitch(
                      ...
                      subtitle:
                          "Changes the default hint text in the message box to display the recipient name",
                      backgroundColor: tileColor,
                      isThreeLine: true,
                    ),
                  ),
                  ...
                  Obx(
                    () => SettingsSwitch(
                      ...
                      subtitle:
                          "Shows contact avatars in direct messages rather than just in group messages",
                      backgroundColor: tileColor,
                      isThreeLine: true,
                    ),
                  ),
                  ...
                  Obx(
                    () => SettingsSwitch(
                      ...
                      subtitle:
                          "Shows replies to the previous message in the thread rather than the original",
                      backgroundColor: tileColor,
                      isThreeLine: true,
                    ),
                  ),
                  ...
                  if (!kIsWeb)
                    const SettingsSubtitle(
                      subtitle:
                          "Note: Overrides any custom avatars set for group chats.",
                    ),
                  if (!kIsWeb)
                    const SettingsDivider(padding: EdgeInsets.only(left: 16)),
                  if (!kIsWeb)
                ...
                Obx(
                  () => SettingsSection(
                    ...
                    children: [
                      SettingsTile(
                        title:
                            "${ss.settings.sendSoundPath.value == null ? "Add" : "Change"} Send Sound",
                        subtitle: ss.settings.sendSoundPath.value != null
                            ? basename(ss.settings.sendSoundPath.value!)
                        ...
                        onTap: () async {
                          ...
                          if (result != null) {
                            ...
                            String path =
                                "${fs.appDocDir.path}/sounds/${"send-"}${platformFile.name}";
                            ...
                            : Row(
                                ...
                                children: [
                                  IconButton(
                                      ...
                                      onPressed: () async {
                                        if (sendPlayer is Player) {
                                          final Player _sendPlayer =
                                              sendPlayer as Player;
                                          if (playingSendSound.value) {
                                            await _sendPlayer.stop();
                                          } else {
                                            await _sendPlayer.setVolume(
                                              ss.settings.soundVolume.value
                                                  .toDouble(),
                                            );
                                            await _sendPlayer.open(
                                              Media(
                                                ss.settings.sendSoundPath
                                                    .value!,
                                              ),
                                            );
                                          }
                                        } else if (sendPlayer
                                            is aw.PlayerController) {
                                          final aw.PlayerController
                                              _sendPlayer =
                                          ...
                                          if (playingSendSound.value) {
                                            ...
                                            if (!sendPrepared) {
                                              await _sendPlayer.preparePlayer(
                                                path: ss.settings.sendSoundPath
                                                    .value!,
                                                volume: ss.settings.soundVolume
                                                        .value
                                                        .toDouble() /
                                                    100,
                                              );
                                  ...
                                  IconButton(
                                    ...
                                    onPressed: () async {
                                      File file = File(
                                        ss.settings.sendSoundPath.value!,
                                      );
                      ...
                      SettingsTile(
                        title:
                            "${ss.settings.receiveSoundPath.value == null ? "Add" : "Change"} Receive Sound",
                        subtitle: ss.settings.receiveSoundPath.value != null
                            ? basename(ss.settings.receiveSoundPath.value!)
                                .substring("receive-".length)
                        ...
                        onTap: () async {
                          ...
                          if (result != null) {
                            PlatformFile platformFile = result.files.first;
                            String path =
                                "${fs.appDocDir.path}/sounds/${"receive-"}${platformFile.name}";
                            await File(path).create(recursive: true);
                            await File(path).writeAsBytes(platformFile.bytes!);
                            ...
                            : Row(
                                ...
                                children: [
                                  IconButton(
                                      ...
                                      onPressed: () async {
                                        if (receivePlayer is Player) {
                                          final Player _receivePlayer =
                                              receivePlayer as Player;
                                          if (playingReceiveSound.value) {
                                            await _receivePlayer.stop();
                                          } else {
                                            await _receivePlayer.setVolume(
                                              ss.settings.soundVolume.value
                                                  .toDouble(),
                                            );
                                            await _receivePlayer.open(
                                              Media(
                                                ss.settings.receiveSoundPath
                                                    .value!,
                                              ),
                                            );
                                          }
                                        } else if (receivePlayer
                                            is aw.PlayerController) {
                                          final aw.PlayerController
                                              _receivePlayer = receivePlayer
                                                  as aw.PlayerController;
                                          if (playingReceiveSound.value) {
                                  ...
                                  IconButton(
                                    ...
                                    onPressed: () async {
                                      File file = File(
                                        ss.settings.receiveSoundPath.value!,
                                      );
                                      if (await file.exists()) {
                                        await file.delete();
                                      }
                      ...
                      const SettingsTile(
                        ...
                        subtitle:
                            "Controls the volume of the send and receive sounds",
                      ),
                      Obx(
                        () => SettingsSlider(
                          ...
                          update: (val) =>
                              ss.settings.soundVolume.value = val.toInt(),
                        ),
                      ),
                    ],
                  ),
                ...
                children: [
                  if (!kIsWeb && !kIsDesktop)
                    Obx(
                      () => SettingsSwitch(
                        ...
                        subtitle:
                            "Automatically open the keyboard when entering a chat",
                        backgroundColor: tileColor,
                      ),
                    ),
                  if (!kIsWeb && !kIsDesktop)
                  ...
                  if (!kIsWeb && !kIsDesktop)
                    Obx(
                      () => SettingsSwitch(
                        ...
                        subtitle:
                            "Swipe down on the message box to hide the keyboard",
                        backgroundColor: tileColor,
                      ),
                    ),
                  if (!kIsWeb && !kIsDesktop)
                  ...
                  if (!kIsWeb && !kIsDesktop)
                    Obx(() => SettingsSwitch(
                          ...
                          subtitle:
                              "Swipe up on the message box to show the keyboard",
                          backgroundColor: tileColor,
                        )),
                  if (!kIsWeb && !kIsDesktop)
                    const SettingsDivider(padding: EdgeInsets.only(left: 16)),
                  ...
                  Obx(
                    () => SettingsSwitch(
                      ...
                      subtitle:
                          "Scroll to the most recent messages in the chat when sending a new text",
                      backgroundColor: tileColor,
                    ),
                  ),
                ],

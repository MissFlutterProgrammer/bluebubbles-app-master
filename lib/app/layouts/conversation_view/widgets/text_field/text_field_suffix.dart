        ...
        return Obx(() {
          bool canSend = widget.textController.text.isNotEmpty ||
              ...
              (widget.controller?.pickedAttachments.isNotEmpty ??
                  false.obs.value);
          bool showRecording =
              (widget.controller?.showRecording.value ?? false.obs.value) &&
                  widget.recorderController != null;
          bool isLinuxArm64 = kIsDesktop &&
              Platform.isLinux &&
              SysInfo.kernelArchitecture == ProcessorArchitecture.arm64;
          return Padding(
            ...
            child: AnimatedCrossFade(
              ...
              firstChild: kIsWeb
                  ...
                  : TextButton(
                      style: TextButton.styleFrom(
                        backgroundColor:
                            !iOS || (iOS && !isChatCreator && !showRecording)
                                ? null
                        ...
                        maximumSize: kIsDesktop
                            ? const Size(40, 40)
                            : const Size(32, 32),
                        minimumSize: kIsDesktop
                            ? const Size(40, 40)
                            : const Size(32, 32),
                          ...
                          : !isChatCreator && !showRecording
                              ? CupertinoIconWrapper(
                                  icon: Icon(
                                    ...
                                    size: iOS
                                        ? 24
                                        : 20, // Waveform icon appears smaller, using size 24
                                  ),
                                )
                              : CupertinoIconWrapper(
                                  icon: Icon(
                      ...
                      onPressed: () async {
                        ...
                        if (widget.controller!.showRecording.value) {
                          if (kIsDesktop) {
                            File temp = File(
                              join(
                                ...
                                "temp",
                                "recorder",
                                "${widget.controller!.chat.guid.characters.where((c) => c.isAlphabetOnly || c.isNumericOnly).join()}.m4a",
                              ),
                            );
                            await RecordPlatform.instance.start(
                                widget.controller!.chat.guid,
                                const RecordConfig(bitRate: 320000),
                                path: temp.path);
                            return;
                          }
                          ...
                          if (kIsDesktop) {
                            path = await RecordPlatform.instance
                                .stop(widget.controller!.chat.guid);
              ...
              secondChild: SendButton(
                ...
                onLongPress: isChatCreator
                    ? () {}
                    : () {
                        if (widget.controller!.scheduledDate.value != null) {
                          return;
                        }
                        sendEffectAction(
                          context,
                          ...
                          widget.textController is MentionTextEditingController
                              ? (widget.textController
                                      as MentionTextEditingController)
                                  .mentionables
                              : [],
                        );
                      },

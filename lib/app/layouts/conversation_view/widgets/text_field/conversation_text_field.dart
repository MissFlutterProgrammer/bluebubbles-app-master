...
class ConversationTextFieldState
    extends CustomState<ConversationTextField, void, ConversationViewController>
  ...
  // previous text state
  TextSelection oldTextFieldSelection =
  ...
  late final emojiColumns = ns.width(context) ~/
      56; // Intentionally not responsive to prevent rebuilds when resizing
    ...
    if (kIsDesktop || kIsWeb) {
      proxyController.addListener(() {
        ...
        TextEditingController realController =
            controller.editing.lastOrNull?.item3 ??
                controller.lastFocusedTextController;
        String text = realController.text;
        TextSelection selection = realController.selection;

        realController.text = text.substring(0, selection.start) +
            emoji +
            text.substring(selection.end);
        realController.selection =
            TextSelection.collapsed(offset: selection.start + emoji.length);

        (controller.editing.lastOrNull?.item3.focusNode ??
                controller.lastFocusedNode)
            .requestFocus();
      });
    }
  }
  ...
  void getTextDraft({String? text}) {
    ...
    if (incomingText != null &&
        ...
        incomingText != controller.textController.text) {
      controller.textController.text = incomingText;
    }
  }

  Future<void> getAttachmentDrafts(
    ...
    final incomingAttachments =
        attachments.isEmpty ? chat.textFieldAttachments : attachments;
    final currentPicked =
        controller.pickedAttachments.map((element) => element.path).toList();
    if (incomingAttachments
        .any((element) => !currentPicked.contains(element))) {
  ...
  void focusListener(bool subject) async {
    final _focusNode =
        subject ? controller.subjectFocusNode : controller.focusNode;
  ...
  void textListener(bool subject) {
    ...
    if (!subject) {
      ...
      if (selection.isCollapsed && selection.start != -1) {
        ...
        final behindMatches =
            MentionTextEditingController.escapingChar.allMatches(behind);
        if (behindMatches.length % 2 != 0) {
          ...
          final aheadMatches =
              MentionTextEditingController.escapingChar.allMatches(ahead);
          ...
          if (oldTextFieldSelection.isCollapsed) {
            if (oldTextFieldSelection.baseOffset > selection.baseOffset) {
              ...
              oldTextFieldSelection =
                  TextSelection.collapsed(offset: behindMatches.last.start);
              controller.textController.selection = oldTextFieldSelection;
              return;
            } else if (oldTextFieldSelection.baseOffset <
                selection.baseOffset) {
              // moving right
              oldTextFieldSelection = TextSelection.collapsed(
          ...
          if (selection.baseOffset - behindMatches.last.end <
              aheadMatches.first.start - selection.baseOffset) {
            // moving left
            oldTextFieldSelection =
                TextSelection.collapsed(offset: behindMatches.last.start);
            controller.textController.selection = oldTextFieldSelection;
      ...
      if (!selection.isCollapsed &&
          oldTextFieldSelection.baseOffset == selection.baseOffset) {
        if (oldTextFieldSelection.extentOffset < selection.extentOffset) {
          ...
          if (aheadMatches.length % 2 != 0) {
            // Assuming the rest of the code works, we're guaranteed to be inside a mention now
            oldTextFieldSelection = TextSelection(
                baseOffset: selection.baseOffset,
                extentOffset: behind.length + aheadMatches.first.end);
            controller.textController.selection = oldTextFieldSelection;
            return;
          }
        } else if (oldTextFieldSelection.extentOffset >
    ...
    // emoji picker
    final _controller =
        subject ? controller.subjectTextController : controller.textController;
    ...
    if (newEmojiText.contains(":")) {
      ...
      if (matches.isNotEmpty &&
          matches.first.start < _controller.selection.start) {
        RegExpMatch match =
            matches.lastWhere((m) => m.start < _controller.selection.start);
        if (newEmojiText[match.end - 1] == ":") {
          ...
          emojiName =
              newEmojiText.substring(match.start + 1, match.end).toLowerCase();
          Iterable<Emoji> emojiExactlyMatches =
              emojiNames.containsKey(emojiName) ? [emojiNames[emojiName]!] : [];
          Iterable<String> emojiNameMatches =
              emojiNames.keys.where((name) => name.startsWith(emojiName));
          Iterable<String> emojiNameAnywhereMatches = emojiNames.keys
              .where((name) => name.substring(1).contains(emojiName))
              .followedBy(emojiFullNames.keys.where((name) =>
                  name.contains(emojiName))); // Substring 1 to avoid dupes
          ...
          allMatches = emojiExactlyMatches
              .followedBy(emojiMatches.followedBy(keywordMatches))
              .toSet()
              .toList();
          ...
          allMatches = controller.mentionables
              .where(
                (e) =>
                    ...
                    e.displayName.toLowerCase().startsWith(
  ...
  Future<void> sendMessage({String? effect}) async {
    ...
    if (controller.scheduledDate.value != null) {
      ...
      if (date.isBefore(DateTime.now())) {
        return showSnackbar("Error", "Pick a date in the future!");
      }
      if (text.contains(MentionTextEditingController.escapingChar)) {
        return showSnackbar(
            "Error", "Mentions are not allowed in scheduled messages!");
      }
      showDialog(
        context: context,
      ...
      final response = await http
          .createScheduled(chat.guid, text, date.toUtc(), {"type": "once"});
      Navigator.of(context).pop();
      if (response.statusCode == 200 && response.data != null) {
        showSnackbar("Notice",
            "Message scheduled successfully for ${buildFullDate(date)}");
      } else {
      ...
      if (text.isEmpty &&
        ...
        if (controller.replyToMessage != null) {
          return showSnackbar("Error",
              "Turn on Private API Attachment Send to send replies with media!");
        } else if (effect != null) {
          return showSnackbar("Error",
              "Turn on Private API Attachment Send to send effects with media!");
        }
      }
      ...
      await controller.send(
        ...
        text,
        controller.subjectTextController.text,
        controller.replyToMessage?.item1.threadOriginatorGuid ??
            controller.replyToMessage?.item1.guid,
            ...
            Row(
              ...
              children: [
                ...
                IconButton(
                  ...
                  visualDensity:
                      Platform.isAndroid ? VisualDensity.compact : null,
                  onPressed: () async {
                    if (kIsDesktop) {
                      ...
                      if (res == null ||
                          res.files.isEmpty ||
                          res.files.first.readStream == null) {
                        return;
                      }
                  ...
                  IconButton(
                      ...
                      onPressed: () async {
                        ...
                        if (gif?.images?.original != null) {
                          final response = await http
                              .downloadFromUrl(gif!.images!.original!.url);
                        ...
                        Positioned(
                          ...
                          child: Obx(
                            () => AnimatedSize(
                              ...
                              curve: controller.showRecording.value
                                  ? Curves.easeOutBack
                                  : Curves.easeOut,
                                                    ...
                                                    child: AnimatedOpacity(
                                                      ...
                                                      opacity:
                                                          (elapsed.inMilliseconds ~/
                                                              ...
                                                              .clamp(0, 1),
                                                      child: Text(
                                                          "Recording... (${prettyDuration(elapsed)})",
                                                          style: context
                                                              .textTheme
                                                              .titleMedium),
                                                    ),
                        ...
                        child: EmojiPicker(
                          ...
                          config: Config(
                            ...
                            bottomActionBarConfig: BottomActionBarConfig(
                              customBottomActionBar: (Config config,
...
class TextFieldComponentState extends State<TextFieldComponent> {
  ...
  Widget build(BuildContext context) {
    ...
    final subjController =
        controller?.subjectTextController ?? subjectTextController;
    return Focus(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      if (!isChatCreator &&
                        ...
                        TextField(
                          ...
                          contentInsertionConfiguration:
                              ContentInsertionConfiguration(
                            onContentInserted: onContentCommit,
                          ),
                        ),
                      if (!isChatCreator &&
                          ss.settings.enablePrivateAPI.value &&
                      ...
                      TextField(
                        ...
                        enableIMEPersonalizedLearning:
                            !ss.settings.incognitoKeyboard.value,
                        textInputAction: ss.settings.sendWithReturn.value &&
                                !kIsWeb &&
                                !kIsDesktop
                        ...
                        decoration: InputDecoration(
                          ...
                          suffixIcon: samsung && !isChatCreator
                              ...
                              : Padding(
                                  padding:
                                      EdgeInsets.only(right: iOS ? 0.0 : 5),
                                  child: TextFieldSuffix(
                                    subjectTextController: subjController,
                                    textController: txtController,
                                    controller: controller,
                        ...
                        contextMenuBuilder: (BuildContext context,
                          ...
                          return AdaptiveTextSelectionToolbar.editableText(
                            editableTextState: editableTextState,
                              ...
                              MentionTextEditingController.escapingRegex
                                      .allMatches(selected)
                                      .length ==
                                  1,
                              [
                                ContextMenuButtonItem(
                                  onPressed: () {
                                    ...
                                    final textPart = text.substring(
                                        0, (end + 1).clamp(1, text.length));
                                    final mentionMatch =
                                    ...
                                    final mentionText = textPart.substring(
                                        mentionMatch.start, mentionMatch.end);
                                    int? mentionIndex = int.tryParse(mentionText
                                        .substring(1, mentionText.length - 1));
                                    if (mentionIndex == null) {
                                      return; // Shouldn't happen
                                    }
                                    final mention =
                                        controller?.mentionables[mentionIndex];
                                    final replacement = mention != null
                                        ? "@${mention.displayName}"
                                        : "";
                                    text = editableTextState
                                        ...
                                        .replaceRange(
                                            (start - 1).clamp(0, text.length),
                                            (end + 1).clamp(min(1, text.length),
                                                text.length),
                                            replacement);
                                    final checkSpace =
                                        end + replacement.length - 1;
                                    final spaceAfter = checkSpace <
                                            text.length &&
                                        text.substring(
                                                end + replacement.length - 1,
                                ...
                                ContextMenuButtonItem(
                                  onPressed: () async {
                                    ...
                                    final spaceAfter = end < text.length &&
                                        text.substring(end, end + 1) == " ";
                                    txtController.selection =
                                        TextSelection.fromPosition(
                                      TextPosition(
                                        offset: end + (spaceAfter ? 1 : 0),
                                      ),
                                    );
                                    editableTextState.hideToolbar();
                                  },
  ...
  void onContentCommit(KeyboardInsertedContent content) async {
    ...
    Logger.info(
        "  -> Content Length: ${content.hasData ? content.data!.length : "null"}");
  ...
  KeyEventResult handleKey(
      FocusNode _, KeyEvent ev, BuildContext context, bool isChatCreator) {
    if (ev is! KeyDownEvent) return KeyEventResult.ignored;

    if ((kIsWeb || Platform.isWindows || Platform.isLinux) &&
        (ev.physicalKey == PhysicalKeyboardKey.keyV ||
            ev.logicalKey == LogicalKeyboardKey.keyV) &&
    ...
    if (ev.logicalKey == LogicalKeyboardKey.arrowDown) {
      if (controller!.mentionSelectedIndex.value <
          controller!.mentionMatches.length - 1) {
        controller!.mentionSelectedIndex.value++;
        if (controller!.mentionSelectedIndex.value >= downMovementIndex &&
            controller!.mentionSelectedIndex <
                controller!.mentionMatches.length -
                    maxShown +
                    downMovementIndex +
                    1) {
      ...
      if (controller!.emojiSelectedIndex.value <
          controller!.emojiMatches.length - 1) {
        controller!.emojiSelectedIndex.value++;
        if (controller!.emojiSelectedIndex.value >= downMovementIndex &&
            controller!.emojiSelectedIndex <
                controller!.emojiMatches.length -
                    maxShown +
                    downMovementIndex +
                    1) {
          controller!.emojiScrollController.jumpTo(max(
              (controller!.emojiSelectedIndex.value - downMovementIndex) * 40,
              controller!.emojiScrollController.offset));
        }
        return KeyEventResult.handled;
      }
      ...
      if (controller!.mentionSelectedIndex.value > 0) {
        ...
        if (controller!.mentionSelectedIndex.value >= upMovementIndex &&
          ...
          controller!.emojiScrollController.jumpTo(min(
              (controller!.mentionSelectedIndex.value - upMovementIndex) * 40,
              controller!.emojiScrollController.offset));
        }
        return KeyEventResult.handled;
      }
        ...
        if (controller!.emojiSelectedIndex.value >= upMovementIndex &&
            controller!.emojiSelectedIndex <
                controller!.emojiMatches.length -
                    ...
                    1) {
          controller!.emojiScrollController.jumpTo(min(
              (controller!.emojiSelectedIndex.value - upMovementIndex) * 40,
              controller!.emojiScrollController.offset));
        }
        return KeyEventResult.handled;
      }
    ...
    if (ev.logicalKey == LogicalKeyboardKey.tab ||
      ...
      if (controller!.focusNode.hasPrimaryFocus &&
        ...
        if (matches.isNotEmpty &&
            matches.any((m) => m.start < textField.selection.start)) {
          RegExpMatch match =
              matches.lastWhere((m) => m.start < textField.selection.start);
      ...
      if (controller!.emojiMatches.length >
          controller!.emojiSelectedIndex.value) {
        ...
        if (matches.isNotEmpty &&
          ...
          String _text =
              "${text.substring(0, match.start)}$char ${text.substring(match.end)}";
          textField.value = TextEditingValue(
              text: _text,
              selection: TextSelection.collapsed(
                  offset: match.start + char.length + 1));
        } else {
      ...
      if (ss.settings.privateSubjectLine.value) {
        if (ev.logicalKey == LogicalKeyboardKey.tab) {
          ...
          if (HardwareKeyboard.instance.isShiftPressed &&
              controller!.focusNode.hasPrimaryFocus) {
            controller!.subjectFocusNode.requestFocus();
            return KeyEventResult.handled;
          }
        }
    ...
    if (ev.physicalKey == PhysicalKeyboardKey.enter &&
        ss.settings.sendWithReturn.value) {
      if (!isNullOrEmpty(textController.text) ||
          !isNullOrEmpty(controller!.subjectTextController.text)) {
        sendMessage();
        controller!.focusNode
            .previousFocus(); // I genuinely don't know why this works
        return KeyEventResult.handled;
      } else {
        controller!.subjectTextController.text = "";
        textController.text =
            ""; // Stop pressing physical enter with enterIsSend from creating newlines
        controller!.focusNode
            .previousFocus(); // I genuinely don't know why this works
        return KeyEventResult.handled;
      }

...
class MaterialHeader extends StatelessWidget implements PreferredSizeWidget {
  ...
  Widget build(BuildContext context) {
    final Rx<Color> _backgroundColor = context.theme.colorScheme.surface
        .withOpacity((kIsDesktop &&
                    ...
                    : () async {
                        ...
                        if (contact == null) {
                          await mcs.invokeMethod("open-contact-form", {
                            ...
                            'address_type':
                                handle.address.isEmail ? 'email' : 'phone'
                          });
                        } else {
                          try {
                            await mcs.invokeMethod(
                                "view-contact-form", {'id': contact.id});
                          } catch (_) {
                            showSnackbar(
                                "Error", "Failed to find contact on device!");
                          }
                        }
                      },
                child: Padding(
                ...
                IconButton(
                  ...
                  onPressed: () {
                    launchUrl(
                      Uri(
                        scheme: "tel",
                        path: controller.chat.participants.first.address,
                      ),
                    );
                  },
                ),
                ...
                child: PopupMenuButton<int>(
                  ...
                  onSelected: (int value) {
                    if (value == 0) {
                      ...
                      controller.chat
                          .toggleArchived(!controller.chat.isArchived!);
        ...
        Positioned(
          child: Obx(
            () => TweenAnimationBuilder<double>(
              ...
              curve: controller.chat.sendProgress.value == 1
                  ? Curves.easeInOut
                  : Curves.easeOutExpo,
              tween: Tween<double>(
                begin: 0,
...
class _ChatIconAndTitleState
    extends CustomState<_ChatIconAndTitle, void, ConversationViewController> {
        ...
        Expanded(
          child: Column(
            ...
            children: [
              ...
              if (samsung &&
                  (controller.chat.isGroup ||
                      (!title.isPhoneNumber && !title.isEmail)) &&
                  !hideInfo)
                Text(
                  controller.chat.isGroup

...
class ServerPayload {
  ...
  bool get isList =>
      (isLegacy && originalJson is List) || (!isLegacy && data is List);
  bool get isString =>
      (isLegacy && originalJson is String) || (!isLegacy && data is String);
  bool get isDict =>
      (isLegacy && originalJson is Map) || (!isLegacy && data is Map);
  ...
  ServerPayload({
    ...
    if ([PayloadEncoding.JSON_OBJECT, PayloadEncoding.JSON_STRING]
            .contains(encoding) &&
        data is String) {
  ...
  factory ServerPayload.fromJson(Map<String, dynamic> json) => ServerPayload(
        originalJson: json,
        data: ((json["data"] ?? json) is String
                ? jsonDecode(json["data"] ?? json)
                : (json["data"] ?? json))
            .cast<String, Object>(),
        ...
        encoding: PayloadEncoding.values.firstWhereOrNull(
                (element) => element.name == json["encoding"]) ??
            PayloadEncoding.JSON_OBJECT,
        encryptionType: EncryptionType.values.firstWhereOrNull(
                (element) => element.name == json["encryptionType"]) ??

...
AttachmentDownloadService attachmentDownloader =
    Get.isRegistered<AttachmentDownloadService>()
...
class AttachmentDownloadService extends GetxService {
  ...
  AttachmentDownloadController? getController(String? guid) {
    return _downloaders.values.flattened
        .firstWhereOrNull((element) => element.attachment.guid == guid);
  }
  ...
  void _addToQueue(AttachmentDownloadController downloader) {
    ...
    final chatGuid =
        downloader.attachment.message.target?.chat.target?.guid ?? "unknown";
  ...
  void _removeFromQueue(AttachmentDownloadController downloader) {
    ...
    final chatGuid =
        downloader.attachment.message.target?.chat.target?.guid ?? "unknown";
    _downloaders[chatGuid]!
        .removeWhere((e) => e.attachment.guid == downloader.attachment.guid);
  ...
  void _fetchNext() {
    if (_downloaders.values.flattened.where((e) => e.isFetching).length <
        maxDownloads) {
      ...
      if (cm.activeChat != null &&
          _downloaders.containsKey(cm.activeChat!.chat.guid)) {
        activeChatDownloader = _downloaders[cm.activeChat!.chat.guid]!
            .firstWhereOrNull((e) => !e.isFetching);
  ...
  Future<void> fetchAttachment() async {
    ...
    if (ss.settings.autoSave.value &&
      ...
      if (attachment.mimeType?.startsWith("image") ?? false) {
        ...
        await File(join(filePath, file.value!.name))
            .writeAsBytes(file.value!.bytes!);
      }
    }
  }

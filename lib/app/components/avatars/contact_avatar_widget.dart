  ...
  void onAvatarTap() async {
    ...
    final Color color = await showColorPickerDialog(
      ...
      widget.handle?.color != null
          ...
          : toColorGradient(widget.handle!.address)[0],
      title: Container(
    ...
    eventDispatcher
        .emit("refresh-avatar", [widget.handle?.address, widget.handle?.color]);
  }
        ...
        child: GestureDetector(
          onTap: !widget.editable || (widget.handle == null && contact == null)
              ...
              : () async {
                  if (contact != null) {
                    await mcs
                        .invokeMethod("view-contact-form", {'id': contact!.id});
                  } else {
                    await mcs.invokeMethod("open-contact-form", {
                      'address': widget.handle!.address,
                      'address_type':
                          widget.handle!.address.isEmail ? 'email' : 'phone'
                    });
                  }
                },
          onLongPress:
              !widget.editable || widget.handle == null ? null : onAvatarTap,
          child: Container(
            ...
            child: Obx(() {
              final hide = ss.settings.redactedMode.value &&
              ...
              if (!hide &&
                ...
                String? initials =
                    widget.handle?.initials?.substring(0, iOS ? null : 1);
                if (!isNullOrEmpty(initials) && !hide) {
                  return Text(
                    initials!,
                    key: Key(
                      "$keyPrefix-avatar-text",
                    ),
                    style: TextStyle(

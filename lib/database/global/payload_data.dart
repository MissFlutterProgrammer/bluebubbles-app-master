    ...
    if (json is Map) {
      return PayloadData(
        ...
        urlData: json["urlData"]
            ?.map((e) => UrlPreviewData.fromJson(e))
            .toList()
            .cast<UrlPreviewData>(),
        appData: json["appData"]
            ?.map((e) => iMessageAppData.fromJson(e))
            .toList()
      ...
      if (sanitized.first["objects"][1] is Map &&
          sanitized.first["objects"][1].containsKey("NS.keys")) {
        ...
        return PayloadData(
          ...
          urlData: data
              .map((e) => UrlPreviewData.fromJson(Map<String, dynamic>.from(e)))
              .toList(),
        );
      }
    }
...
class MediaMetadata {
  ...
  factory MediaMetadata.fromJson(Map<String, dynamic> json) {
    ...
    if (json["size"] is String && json["size"].contains(",")) {
      size = Size(
          double.parse(json["size"].split(",").first.toString().numericOnly()),
      ...
      Logger.warn(
          "Failed to parse media metadata size: ${json["size"].toString()}");
    }
...
class iMessageAppData {
  ...
  factory iMessageAppData.fromJson(Map<String, dynamic> json) =>
      iMessageAppData(
        appName: json["an"],
        ldText: json["ldtext"],
...
dynamic replaceDollar<T>(T element, {bool isValue = false}) {
  ...
  if (element is List) {
    ...
    for (MapEntry item in element.entries) {
      newMap[replaceDollar(item.key)] =
          replaceDollar(item.value, isValue: true);
    }
    return newMap;
...
dynamic extractUIDs<T>(T element, List objects) {
  ...
  if (element is List) {
    ...
    } else if (element.containsKey("NS.keys") &&
        element.containsKey("NS.objects")) {

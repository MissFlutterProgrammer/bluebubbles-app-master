...
class _PickedAttachmentsHolderState
    extends OptimizedState<PickedAttachmentsHolder> {
  ...
  void selectMention(int index, bool custom) async {
    ...
    if (matches.isNotEmpty &&
        matches.any((m) => m.start < _controller.selection.start)) {
      final match =
          ...
          Obx(() {
            if (widget.controller!.emojiMatches.isNotEmpty) {
              return ConstrainedBox(
                constraints: BoxConstraints(
                  maxHeight: min(
                    widget.controller!.emojiMatches.length * 60,
                    180,
                  ),
                ),
                child: Padding(
                    ...
                    child: Scrollbar(
                      ...
                      child: ListView.builder(
                        ...
                        findChildIndexCallback: (key) => findChildIndexByKey(
                          widget.controller!.emojiMatches,
                          key,
                          (item) => item.shortName,
                        ),
                        itemBuilder: (BuildContext context, int index) =>
                          ...
                          child: InkWell(
                            onTapDown: (details) {
                              widget.controller!.emojiSelectedIndex.value =
                                  index;
                            },
                            onTap: () {
                              final _controller =
                                  widget.controller!.lastFocusedTextController;
                              final text = _controller.text;
                              final regExp = RegExp(r":[^: \n]+([ \n]|$)",
                                  multiLine: true);
                              final matches = regExp.allMatches(text);
                              if (matches.isNotEmpty &&
                                  matches.any((m) =>
                                      m.start < _controller.selection.start)) {
                                final match = matches.lastWhere((m) =>
                                    m.start < _controller.selection.start);
                                final char =
              ...
              return ConstrainedBox(
                constraints: BoxConstraints(
                  maxHeight: min(
                    widget.controller!.mentionMatches.length * 60,
                    180,
                  ),
                ),
                child: Padding(
                  ...
                  child: Container(
                    ...
                    child: Scrollbar(
                      ...
                      child: ListView.builder(
                        ...
                        shrinkWrap: true,
                        findChildIndexCallback: (key) => findChildIndexByKey(
                            widget.controller!.mentionMatches,
                            key,
                            (item) => item.address),
                        itemBuilder: (BuildContext context, int index) =>
                            Material(
                          ...
                          child: InkWell(
                            onTapDown: (details) {
                              widget.controller!.mentionSelectedIndex.value =
                                  index;
                            },
                            onTap: () {
                              ...
                              () => ListTile(
                                ...
                                title: Row(
                                  ...
                                  children: <Widget>[
                                    ...
                                    Text(
                                      widget.controller!.mentionMatches[index]
                                          .displayName,
                                      style: context.textTheme.labelLarge!,
                                      overflow: TextOverflow.ellipsis,
                                    ),
                                    if (widget.controller!.mentionMatches[index]
                                            .displayName !=
                                        widget.controller!.mentionMatches[index]
                                            .address)
                                      const SizedBox(width: 8),

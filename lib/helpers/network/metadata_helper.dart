...
class MetadataHelper {
  ...
  static Future<Metadata?> fetchMetadata(Message message) async {
    ...
    try {
      ...
      Logger.error('An error occurred while fetching URL Preview Metadata!',
  ...
  static Future<Metadata> _manuallyGetMetadata(String url) async {
    ...
    try {
      ...
      final props = document.head?.children
              .where((e) =>
                  e.localName == "meta" &&
                  e.attributes["property"].toString().contains("og:"))
              .map((e) =>
                  MapEntry(e.attributes["property"], e.attributes["content"]))
              .toList() ??
          [];

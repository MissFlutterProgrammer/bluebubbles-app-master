...
class _MessagePropertiesState
    extends CustomState<MessageProperties, void, MessageWidgetController> {
  Message get message => controller.message;
  MessagesService get service =>
      ms(controller.cvController?.chat.guid ?? cm.activeChat!.chat.guid);
  ...
  List<TextSpan> getProperties() {
    ...
    final replyList = service.struct
        .threads(message.guid!, widget.part.part, returnOriginator: false);
    if (message.expressiveSendStyleId != null) {
      final effect = effectMap.entries
              .firstWhereOrNull(
                (element) => element.value == message.expressiveSendStyleId,
              )
              ?.key ??
          "unknown";
      properties.add(
            ...
            recognizer: TapGestureRecognizer()
              ..onTap = () {
                if (stringToMessageEffect[effect] == MessageEffect.echo) {
                  showSnackbar("Notice",
                      "Echo animation is not supported at this time.");
                  return;
                }
                HapticFeedback.mediumImpact();
                if ((stringToMessageEffect[effect] ?? MessageEffect.none)
                    .isBubble) {
                  eventDispatcher.emit('play-bubble-effect',
                      '${widget.part.part}/${message.guid}');
    ...
    if (replyList.isNotEmpty) {
      properties.add(
        TextSpan(
            text:
                "${replyList.length} repl${replyList.length > 1 ? "ies" : "y"}",
            recognizer: TapGestureRecognizer()
              ..onTap = () {
      ...
      child: props.isNotEmpty
          ? Padding(
              padding: const EdgeInsets.symmetric(horizontal: 15)
                  .add(const EdgeInsets.only(top: 3)),
              child: Text.rich(
                TextSpan(
                  children:
                      intersperse(const TextSpan(text: " • "), props).toList(),
                ),

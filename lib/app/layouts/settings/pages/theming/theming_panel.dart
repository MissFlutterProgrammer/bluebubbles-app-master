  ...
  void onReady() async {
    ...
    if (!kIsWeb && !kIsDesktop) {
      updateObx(() async {
        ...
        refreshRates.value =
            modes.map((e) => e.refreshRate.round()).toSet().toList();
        currentMode.value =
            (await ss.settings.getDisplayMode()).refreshRate.round();
      });
    }
  }
}
...
class _ThemingPanelState
    extends CustomState<ThemingPanel, void, ThemingPanelController> {
  @override
                          ...
                          onChanged: (val) async {
                            ...
                            if (currentTheme.name == "Music Theme ☀" ||
                              ...
                              await ts.changeTheme(context,
                                  light: previousLight, dark: previousDark);
                            }
                            ...
                            if (val) {
                              ...
                              try {
                                ...
                                await ss.prefs.setString(
                                    "previous-light", currentLight.name);
                                await ss.prefs.setString(
                                    "previous-dark", currentDark.name);
                                await ts.changeTheme(context,
                                    light: allThemes.firstWhere((element) =>
                                        element.name == "Music Theme ☀"),
                                    dark: allThemes.firstWhere((element) =>
                                        element.name == "Music Theme 🌙"));
                              ...
                              var allThemes = ThemeStruct.getThemes();
                              final lightName =
                                  ss.prefs.getString("previous-light");
                              final darkName =
                                  ss.prefs.getString("previous-dark");
                              var previousLight = allThemes
                                  .firstWhere((e) => e.name == lightName);
                              var previousDark = allThemes
                                  .firstWhere((e) => e.name == darkName);
                    ...
                    Obx(() {
                      if (!fs.fontExistsOnDisk.value) {
                        return SettingsTile(
                          onTap: () async {
                            if (kIsWeb) {
                              try {
                                ...
                                final txn = fs.webDb
                                    .transaction("BBStore", idbModeReadWrite);
                                final store = txn.objectStore("BBStore");
                                await store.put(
                                    res.files.first.bytes!, "iosFont");
                                await txn.completed;

                                final fontLoader =
                                    FontLoader("Apple Color Emoji");
                                final cachedFontBytes = ByteData.view(
                                    res.files.first.bytes!.buffer);
                                fontLoader.addFont(
                            ...
                            if (response.statusCode == 200) {
                              try {
                                ...
                                final file =
                                    File("${fs.appDocDir.path}/font/apple.ttf");
                                await file.create(recursive: true);
                                await file.writeAsBytes(data);
                                fs.fontExistsOnDisk.value = true;
                                final fontLoader =
                                    FontLoader("Apple Color Emoji");
                                final cachedFontBytes =
                                ...
                                Logger.error("Failed to load font!",
                                    error: e, trace: stack);
                                showSnackbar("Error",
                                    "Failed to load font! Error: ${e.toString()}");
                              }
                            } else {
                          ...
                          subtitle: kIsWeb
                              ? "Upload your ttf emoji file into BlueBubbles"
                              : null,
                        );
                      } else {
                        return const SizedBox.shrink();
                      ...
                      if (fs.fontExistsOnDisk.value) {
                        return SettingsTile(
                          onTap: () async {
                            ...
                            showSnackbar("Notice",
                                "Font removed, restart the app for changes to take effect");
                          },

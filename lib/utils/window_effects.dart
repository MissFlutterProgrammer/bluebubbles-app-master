...
class WindowEffects {
  ...
  static List<WindowEffect> get effects => _effects.where((effect) {
        ...
        return version >= _versions[effect]!.item1! &&
            (_versions[effect]!.item2 == null ||
                (version <= _versions[effect]!.item2!));
      }).toList();

  static final _descriptions = {
    WindowEffect.tabbed:
        "Tabbed is a Mica-like material that incorporates theme and desktop wallpaper, but is more "
            "sensitive to desktop wallpaper color. Works only on later Windows 11 versions (build 22523 or higher).",
    WindowEffect.mica:
        "Mica is an opaque, dynamic material that incorporates theme and desktop wallpaper to paint "
    ...
    WindowEffect.acrylic:
        "Acrylic is a type of brush that creates a translucent texture. You can apply acrylic to "
  ...
  static Map<WindowEffect, String> get descriptions {
    return Map.fromEntries(
        effects.map((effect) => MapEntry(effect, _descriptions[effect] ?? "")));
  }
  ...
  static Future<void> setEffect({required Color color}) async {
    ...
    bool addOpacity = ss.settings.windowEffect.value == WindowEffect.acrylic &&
        !supportsTransparentAcrylic;
    ...
    if (_dependencies[effect]?.contains(EffectDependencies.brightness) ??
        false) {
      _dark = isDark(color: color);
    }
    await Window.setEffect(
        effect: effect,
        color: color.withOpacity(addOpacity ? _extra : 0),
        dark: _dark);
  }
}
...
int parsedWindowsVersion() {
  ...
  return int.tryParse(
          raw.substring(raw.indexOf("Build") + 6, raw.length - 1)) ??
      0;
}

...
class _FindMyPageState extends OptimizedState<FindMyPage>
  ...
  void initState() {
    ...
    socket.socket.on("new-findmy-location", (data) {
      try {
        ...
        final existingFriendIndex = friends.indexWhere((e) =>
            e.handle?.uniqueAddressAndService ==
            friend.handle?.uniqueAddressAndService);
        final existingFriend =
            existingFriendIndex == -1 ? null : friends[existingFriendIndex];
  ...
  void getLocations(
      {bool refreshFriends = true, bool refreshDevices = false}) async {
    if (!(Platform.isLinux && !kIsWeb)) {
      ...
      if (granted == LocationPermission.whileInUse ||
        ...
        Geolocator.getCurrentPosition().then((loc) {
          ...
          if (!refreshFriends) {
            mapController.move(
                LatLng(location!.latitude, location!.longitude), 10);
          }
        });
      }
    }
    ...
    final response2 = refreshFriends
        ? await http.refreshFindMyFriends().catchError((_) async {
            ...
            showSnackbar("Error",
                "Something went wrong refreshing FindMy Friends data!");
    ...
    if (response2.statusCode == 200 && response2.data['data'] != null) {
      try {
        ...
        Logger.error("Failed to parse FindMy Friends location data!",
            error: e, trace: s);
        setState(() {
          fetching2 = null;
          refreshing2 = false;
    ...
    final response = refreshDevices
        ? await http.refreshFindMyDevices().catchError((_) async {
            ...
            showSnackbar("Error",
                "Something went wrong refreshing FindMy Devices data!");
            return Response(requestOptions: RequestOptions(path: ''));
          })
    ...
    if (response.statusCode == 200 && response.data['data'] != null) {
      try {
        ...
        Logger.error("Failed to parse FindMy Devices location data!",
            error: e, trace: s);
        setState(() {
          fetching = null;
          refreshing = false;
  ...
  void buildFriendMarker(FindMyFriend friend) {
    markers[friend.handle?.uniqueAddressAndService ?? randomString(6)] = Marker(
      key: ValueKey(
        'friend-${friend.handle?.uniqueAddressAndService ?? randomString(6)}',
      ),
  ...
  Widget build(BuildContext context) {
    final devicesWithLocation = devices
        .where((item) =>
            (item.address?.label ?? item.address?.mapItemFullAddress) != null &&
      ...
      SliverList(
        delegate: SliverChildListDelegate(
          [
            if (fetching == null ||
                fetching == true ||
                (fetching == false && devices.isEmpty))
              Center(
                child: Padding(
                  ...
                  Material(
                    ...
                    child: ListView.builder(
                      ...
                      findChildIndexCallback: (key) => findChildIndexByKey(
                          devicesWithLocation,
                          key,
                          (item) => item.address?.uniqueValue),
                      itemBuilder: (context, i) {
                        final item = devicesWithLocation[i];
                        return ListTile(
                          key: ValueKey(item.address?.uniqueValue),
                          mouseCursor: MouseCursor.defer,
                          title: Text(ss.settings.redactedMode.value
                              ? "Device"
                              : (item.name ?? "Unknown Device")),
                          subtitle: Text(ss.settings.redactedMode.value
                              ? "Location"
                              : (item.address?.label ??
                                  item.address?.mapItemFullAddress ??
                                  "No location found")),
                          onTap: item.location?.latitude != null &&
                              ...
                              : null,
                          trailing: item.location?.latitude != null &&
                                  item.location?.longitude != null
                              ? ButtonTheme(
                                  minWidth: 1,
                                  child: TextButton(
      ...
      SliverList(
        delegate: SliverChildListDelegate(
          [
            if (fetching2 == null ||
                fetching2 == true ||
                (fetching2 == false && friends.isEmpty))
              Center(
                child: Padding(
                  padding: const EdgeInsets.only(top: 100),
                  child: Column(
                  ...
                  Material(
                    ...
                    child: ListView.builder(
                      ...
                      findChildIndexCallback: (key) => findChildIndexByKey(
                          friendsWithLocation,
                          key,
                          (item) => item.handle?.uniqueAddressAndService),
                      itemBuilder: (context, i) {
                        ...
                        return ListTile(
                          ...
                          title: Text(item.handle?.displayName ??
                              item.title ??
                              "Unknown Friend"),
                          subtitle: Text(ss.settings.redactedMode.value
                              ? "Location"
                          ...
                          onTap: () async {
                            ...
                            final marker = markers.values.firstWhere((e) =>
                                e.point.latitude == item.latitude &&
                                e.point.longitude == item.longitude);
                            popupController.showPopupsOnlyFor([marker]);
                            mapController.move(
                                LatLng(item.latitude!, item.longitude!), 10);
                          },
                          onLongPress: () async {
                            const encoder = JsonEncoder.withIndent("     ");
                            final str = encoder.convert(item.toJson());
                                ...
                                ClipRect(
                                  child: BackdropFilter(
                                    ...
                                    child: Container(
                                      height: appWindow.titleBarHeight,
                                      width:
                                          appWindow.titleBarButtonSize.width *
                                              3,
                                      color: context
                                          .theme.colorScheme.properSurface
                                          .withOpacity(0.5),
      ...
      tabs: [
        Container(
          ...
          child: Column(
            ...
            children: [
              Icon(
                iOS ? CupertinoIcons.device_desktop : Icons.devices,
              ),
              const Text("Devices"),
            ],
          ),
        ...
        Container(
          ...
          child: Column(
            ...
            children: [
              Icon(
                iOS ? CupertinoIcons.person_2 : Icons.person,
              ),
              const Text("Friends"),
            ],
          ),
  ...
  Widget buildNormal(BuildContext context, List<SliverList> devicesBodySlivers,
      List<SliverList> friendsBodySlivers) {
    return Obx(
      () => Scaffold(
                ...
                children: <Widget>[
                  NotificationListener<ScrollEndNotification>(
                    onNotification: (_) {
                      ...
                      if (controller1.offset > 0 &&
                          controller1.offset < scrollDistance) {
                        final double snapOffset =
                            controller1.offset / scrollDistance > 0.5
                                ? scrollDistance
                                : 0;
                    ...
                    child: ScrollbarWrapper(
                      ...
                      child: Obx(
                        () => CustomScrollView(
                          ...
                          slivers: <Widget>[
                            if (samsung)
                              buildSamsungAppBar(context, "FindMy Devices"),
                            if (ss.settings.skin.value != Skins.Samsung)
                              ...devicesBodySlivers,
                            if (ss.settings.skin.value == Skins.Samsung)
                              SliverToBoxAdapter(
                  ...
                  NotificationListener<ScrollEndNotification>(
                    onNotification: (_) {
                      ...
                      if (controller2.offset > 0 &&
                          controller2.offset < scrollDistance) {
                        final double snapOffset =
                            controller2.offset / scrollDistance > 0.5
                                ? scrollDistance
                                : 0;
    ...
    return SliverAppBar(
      ...
      flexibleSpace: LayoutBuilder(
        builder: (context, constraints) {
          var expandRatio =
              (constraints.maxHeight - 50) / (context.height / 3 - 50);
            ...
            popupDisplayOptions: PopupDisplayOptions(
              builder: (context, marker) {
                ...
                if (key?.value.contains("device")) {
                  ...
                  final item = friends.firstWhere((e) =>
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          if (item.lastUpdated != null &&
                              item.status != LocationStatus.live)

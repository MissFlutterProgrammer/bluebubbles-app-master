...
List<InlineSpan> buildMessageSpans(
  ...
  if (part.mentions.isNotEmpty) {
    part.mentions.forEachIndexed((i, e) {
      ...
      textSpans.addAll(MessageHelper.buildEmojiText(
        part.displayText!.substring(
            i == 0 ? 0 : part.mentions[i - 1].range.last, range.first),
        textStyle,
      ));
      textSpans.addAll(MessageHelper.buildEmojiText(
          ...
          recognizer: TapGestureRecognizer()
            ..onTap = () async {
              ...
              final handle = cm.activeChat!.chat.participants.firstWhereOrNull(
                  (e) => e.address == part.mentions[i].mentionedAddress);
...
Future<List<InlineSpan>> buildEnrichedMessageSpans(
    ...
    if (!kIsWeb && !kIsDesktop && ss.settings.smartReply.value) {
      if (controller.mlKitParsedText["${message.guid!}-${part.part}"] == null) {
        try {
          controller.mlKitParsedText["${message.guid!}-${part.part}"] =
              await GoogleMlKit.nlp
                  .entityExtractor(EntityExtractorLanguage.english)
      ...
      for (EntityAnnotation element in normalizedEntities) {
        if (element.entities.first is AddressEntity) {
          linkIndexMatches
              .add(Tuple3("map", [element.start, element.end], null));
        } else if (element.entities.first is PhoneEntity) {
          linkIndexMatches
              .add(Tuple3("phone", [element.start, element.end], null));
        } else if (element.entities.first is EmailEntity) {
          linkIndexMatches
              .add(Tuple3("email", [element.start, element.end], null));
        } else if (element.entities.first is UrlEntity) {
          linkIndexMatches
              .add(Tuple3("link", [element.start, element.end], null));
          ...
          linkIndexMatches.add(Tuple3("tracking", [element.start, element.end],
          ...
          linkIndexMatches.add(Tuple3("flight", [element.start, element.end],
              [ent.airlineCode, ent.flightNumber]));
        } else if (element.entities.first is MentionEntity) {
          linkIndexMatches.add(Tuple3("mention", [element.start, element.end],
              [element.entities.first.rawValue]));
        }
      }
    } else {
      ...
      linkIndexMatches.addAll(part.mentions.map((e) => Tuple3("mention",
          [e.range.first, e.range.last], [e.mentionedAddress ?? ""])));
    }
  }
  // render subject
  if (!isNullOrEmpty(part.subject)) {
  ...
  if (linkIndexMatches.isNotEmpty) {
    linkIndexMatches.forEachIndexed((i, e) {
      ...
      textSpans.addAll(MessageHelper.buildEmojiText(
        part.displayText!.substring(
            i == 0 ? 0 : linkIndexMatches[i - 1].item2.last, range.first),
        textStyle,
      ));
      if (type == "mention") {
        textSpans.addAll(MessageHelper.buildEmojiText(
            text, textStyle.apply(fontWeightDelta: 2),
            recognizer: TapGestureRecognizer()
              ..onTap = () async {
                if (kIsDesktop || kIsWeb) return;
